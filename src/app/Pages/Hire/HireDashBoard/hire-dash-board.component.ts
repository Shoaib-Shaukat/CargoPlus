...
import { requestSearch, getDashBoardData, getDetail } from './DashBoard'
...
@Component({
  selector: 'app-hire-dash-board',
  templateUrl: './hire-dash-board.component.html',
  styleUrls: ['./hire-dash-board.component.css']
})
export class HireDashBoardComponent implements OnInit {
    ...
    this.API.getdata("/Setups/getAirLines").subscribe(
      (c) => {
        if (c != null) {
          ...
          this.dashBoardForm.controls.ALCode.setValue(0);
        }
      },
      (error) => {
        Swal.fire({
  ...
  searchHireIn() {
    ...
    if (this.validForm == true) {
      ...
      this.requestSearch.ALCode = this.dashBoardForm.controls.ALCode.value;
      ...
      this.API.PostData('/Hire/getDashBoardData', this.requestSearch).subscribe(c => {
        if (c != null) {
  ...
  getDetailReport() {
    ...
    if (this.validForm == true) {
      ...
      this.requestSearch.ToDate = this.dashBoardForm.controls.ToDate.value;
      this.requestSearch.gsecatID = this.dashBoardForm.controls.gsecatID.value;
      this.API.PostData('/Hire/getDetail', this.requestSearch).subscribe(c => {
        if (c != null) {
